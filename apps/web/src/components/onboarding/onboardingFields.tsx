  ...
  return (
    <Controller
      ...
      render={({ field, fieldState }) => (
        ...
        >
          <FieldLabel className="text-sm hidden sm:block">
  ...
  const checkUsername = async () => {
    ...
    try {
      const res = await api.get(
        `/users/check-username?username=${getValues("username")}`,
      );
          ...
          <div className="rounded-xl border border-border/50 bg-muted/20 p-3 space-y-3 mt-2 overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <div className="flex gap-2">
                ...
                >
                  {isGenerating ? (
                    <Loader inline className="mr-1 size-3" />
                  ) : (
                ...
                {generatedNames.map((name) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={cn(
                        "px-3 py-1.5 text-sm rounded-lg border transition-colors outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50 active:scale-95",
                        isSelected
...
export function InterestsField({
  ...
  const { control, trigger } = useFormContext<initiateProfileType>();
