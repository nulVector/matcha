...
import { dbBufferCommittedRowsCounter, jobDurationHistogram } from "../config/metrics";

export const dbBufferWorker = new Worker(
  QueueName.DB_BUFFER,
  async (job: Job) => {
    const endTimer = jobDurationHistogram.labels(QueueName.DB_BUFFER,job.name).startTimer();
    const traceId = job.id || createId();
    try{
      return traceStorage.run({ traceId }, async () => {
        ...
        switch (task.name) {
          case JobName.PROCESS_MESSAGE_BATCH: {
            ...
            logger.info({ batchSize: messages.length, originalTraces: traceIds }, "Flushing message batch to Postgres");
            ...
            dbBufferCommittedRowsCounter.labels('messages').inc(messagesToInsert.length);
          ...
          case JobName.PROCESS_READ_BATCH: {
            ...
            logger.info({ batchSize: readEntries.length, originalTraces: traceIds }, "Flushing read receipts to Postgres");
            if (pgConnIds.length > 0) {
              ...
              dbBufferCommittedRowsCounter.labels('read_receipts').inc(pgConnIds.length);
            }
            break;
          }
          default: {
...
dbBufferWorker.on("failed", (job, err) => {
  ...
  traceStorage.run({ traceId }, () => {
    logger.error({ err, jobName: job?.name }, "DbBuffer job failed");
  });
});
